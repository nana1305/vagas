...
void callback(char* topic, byte * payload, unsigned int length) {
  ...
  for (contador = 0 ; contador <= qtdVagas ; contador++) {
    ...
    // vagas[contador]
    // OS COLCHETES "[]" SERVEM PARA PEGAR O VALOR NUMA POSIÇÃO ESPECIFICA DO VETOR/ARRAY
    // A VARIAVEL "[contador]" REPRESENTA A POSIÇÃO ATUAL DO LOOP EX.: 0,1...
    // LOGO SE EU USAR O CÓDIGO vagas[contador] e o valor do contador for 2, eu estou acessando
    // o valor na posição "2" do vetor/array de vagas
    if (vagas[contador] == 1) {
      livres = livres + 1;
